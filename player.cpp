...
struct line_result {
    ...
    InstantCG::ColorRGB color;
};

player::player(const int START_X, const int START_Y, const int FOV, const int SCREEN_HEIGHT) {
...
line_result player::doRayCast() {
    for (int x = 0; x < MAPX; x++){
        ...
        {
            case 1:  color = InstantCG::RGB_Red;  break; //red
            case 2:  color = InstantCG::RGB_Green;  break; //green
            case 3:  color = InstantCG::RGB_Blue;   break; //blue
            case 4:  color = InstantCG::RGB_White;  break; //white
            default: color = InstantCG::RGB_Yellow; break; //yellow
        }
...
void player::calcStepAndSideDist(const double RAY_DIR_X, const double RAY_DIR_Y, const double DELTA_X,
                                 const double DELTA_Y, int *stepX, int *stepY, double *sideDistX,
                                 double *sideDistY) {
    if (RAY_DIR_X < 0){
        *stepX = 1;
